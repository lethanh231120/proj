...
const Content = ({ postType }) => {
  ...
  useEffect(() => {
    ...
    const callData = async() => {
      ...
      const { data } = await axios.post('https://ulatroi.net/api/auth',
        formData2, 
        {
          headers: {
            ...
            >
              {postType && postType === TEXT ? (
                <>
                  ...
                  <a href={`${item.url}`} target='_blank' className='content-item-url'></a>
                ...
                <>
                    <div className={`${item?.postFile !== '' ? '' : 'content-video-none'}`}>
                      <a href={`${item.url}`} target='_blank' className='content-video-url'></a>
                  ...
                  {/* ) : (
                    <div className='content-video'>
                      không có video
                      <video className='content-video-item' autoplay controls>
                        <source src={item?.postFile !== '' && item?.postFile} type={`${item?.postFile !== '' && 'video/mp4' }`}/>
                      </video>
                    </div>
                  )}
